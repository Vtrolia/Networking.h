...
//  utilizes the winsock library to make Windows networking programs. If you're using a Unix-based system, download the regular 'Networking.h'. This library is //
//  functionally the same as the other one, it's just that this one is needed to work with the different networking commands that come with Windows. All you    //
//  have to do once you import this library is run either connect_to_server() or connect_to_client() and you're good to go. This library's job is to set up     //
//  the connection for you, as setting up send() and receive() is often very dependant on your specific use case, and I don't want to force you to work around  //
...
/*
 * This is just a wrapper for the winsock close connection function. This makes the call to close the connection the same as the Unix version, so that you don't
 * have to change anything in the code when making a Windows port for your program.
 * @returns: the integer returned by the WSACleanup() function
 */
int close(int sock)
{
    closesocket(sock);
